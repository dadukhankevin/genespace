...
class Landscape(enum.Enum):
    TINY = 1  # Many solutions
    MEDIUM = 2  # Only one valid solution
    LARGE = 3
...
class GeneralEvolution:
    def __init__(self, fitness_function, output_shape: tuple[int, ...], scale: float=1, input_length: int = 250):
        ...
        """
        self.scale = scale
        self.hidden_size = 2000 * (self.scale ** 2)
    ...
    def solve(self, generations=300, backprop_mode=environments.BackpropMode.GRADIENT_DESCENT, backprop_every_n=10, epochs=1, selection_percent=.4, batch_size=32):
        ...
        self.environment.evolve(generations=generations, backprop_mode=backprop_mode, backprop_every_n=backprop_every_n * self.scale, epochs=epochs, selection_percent=selection_percent, batch_size=batch_size * self.scale)
